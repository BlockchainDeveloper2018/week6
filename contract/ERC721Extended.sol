...
contract ERC721Token is ERC721, ERC721BasicToken {
    ...
    function createNewToken(string _data) public payable {
        ...
        uint _time = uint(sha3(uint(now)));
        uint _tokensSha;
        uint _lastTokenIndex = allTokens.length.sub(1);
        uint _lastToken = allTokens[_lastTokenIndex];
        for (uint i = 1; i <= _lastToken; i++) {
            _tokensSha += uint(sha3(allTokensIndex[i]));
        }
        uint _dataSha = uint(sha3(_data));
        uint tokenId = uint(sha3(_time + _tokensSha + _dataSha));
